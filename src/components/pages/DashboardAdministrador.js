...
export default function DashboardAdministrador() {
  ...
  const LoginADM2 = "vitormartinscruz04@gmail.com";
  const LoginADM3 = "luccaleca@gmail.com";
  ...
  if (
    ...
    ((VerificaADM === LoginADM1) === LoginADM2) === LoginADM3
  ) {
    supAuth = true;
  }
  ...
  useEffect(() => {
    const getVendaTotalEmpDia = async () => {
      //console.log(id);
      await fetch(
        "http://192.168.30.252:9091/app_portal_server/Backend/api_dashboard_adm_auto/box/api_dashboard_box_vendas_dia.php"
      )
        .then((response) => response.json())
        .then((responseJson) => {
          setDataVendaTotalEmpDia(responseJson.BoxVenda);
